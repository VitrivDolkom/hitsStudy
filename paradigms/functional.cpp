...
Node *createNode(Node *getBalancedTreeRoot(vector<int> numbers, Node **refs, int start, int end),
                 vector<int> numbers, Node **refs, int start, int end)
{

  return new Node(numbers[getMiddle(start, end)],
                  createNodeChild(getBalancedTreeRoot, numbers, refs, refs[getMiddle(start, end)], start,
                                  getMiddle(start, end) - 1),
                  createNodeChild(getBalancedTreeRoot, numbers, refs, refs[getMiddle(start, end)],
                                  getMiddle(start, end) + 1, end));
}
...
template <typename T>
bool compareVariableWithValue(T var, T value)
{
  return var == value;
}
...
vector<int> recursionInput(vector<int> numbers)
{
  ...
  return !compareVariableWithValue(readValue<char>(), ',') ? pushToVector(numbers, number) : recursionInput(pushToVector(numbers, number));
}
